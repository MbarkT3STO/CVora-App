...
import { ok, err, respond, createToken, getTokenExpiresAt } from './_utils';
...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  const { username, password, action } = JSON.parse(event.body || '{}') as {
    username: string;
    password: string;
  ...
  if (action === 'register') {
    ...
    // Store hashed password — simple hash for demo; use bcrypt in production
  ...
  // Also support legacy hardcoded admin for backwards compat
